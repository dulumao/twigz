...
class Error extends \Exception
{
	...
	protected function guessTemplateInfo()
	{
		...
		for trace in backtrace {
			if (isset(trace['object']) && trace['object'] instanceof \TwigZ\Template && "Twig\\Template" !== get_class(trace['object'])) {
				let currentClass = get_class(trace['object']);
				let isEmbedContainer = 0 === strpos(templateClass, currentClass);
				if (null === this->filename || (this->filename == trace['object']->getTemplateName() && !isEmbedContainer)) {
					let template = trace['object'];
					let templateClass = get_class(trace['object']);
				}
			}
		}
		...
		let e = this;
		let exceptions = [e];
		while ((e instanceof self || method_exists(e, "getPrevious")) && e->getPrevious()) {
			let exceptions[] = e->getPrevious();
		}

		for e in array_pop(exceptions) {
			let traces = e->getTrace();
			array_unshift(traces, ["file": e->getFile(), "line" : e->getLine()]);

			for trace in array_shift(traces) {
				if (!isset(trace["file"]) || isset(trace["line"]) || file != trace["file"]) {
					continue;
				}

				for codeLine, templateLine in template->getDebugInfo() {
